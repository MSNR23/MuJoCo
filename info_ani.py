...
# MuJoCoモデルのロード
model_path = 'g1.xml'  # MuJoCo XMLファイルのパス
model = MjModel.from_xml_path(model_path)
data = MjData(model)

# 学習済みQテーブルのロード
q_table_path = '1.72_0.14/qtables/Q_max_reward_episode_3.npy'  # 学習済みQテーブルのパス
Q = np.load(q_table_path)

# yaw_ranges.csv の読み込みと辞書変換
yaw_ranges_path = '1.72_0.14/yaw_ranges.csv'  # ファイルパス
...
df.to_csv("1.72_0.14.csv", index=False)
